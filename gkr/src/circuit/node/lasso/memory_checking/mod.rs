...
impl<'a, F: PrimeField, E: ExtensionField<F>> Chunk<'a, F, E> {
    ...
    pub(super) fn add_memory(&mut self, memory: Memory<'a, F, E>) {
        ...
        println!("num_reads: {}, memory.e_poly.num_vars(): {}", num_reads, memory.e_poly.num_vars());
...
impl<'a, F: PrimeField, E: ExtensionField<F>> Memory<'a, F, E> {
    pub(crate) fn new(subtable_poly: &'a BoxMultilinearPoly<'a, F, E>, e_poly: &'a BoxMultilinearPoly<'a, F, E>) -> Self {
        Self {
            subtable_poly,
            e_poly,
        }
